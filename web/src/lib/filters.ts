...
export type Filters = {
  ...
  audience: 'all' | 'pipeline' | 'inventory'; eastBay: boolean; nonPriority: boolean; sort: string; dir: 'asc' | 'desc';
};
export const DEFAULT: Filters = { q: '', nb: [], type: [], tag: [], kitchen: '', status: [], outreach: [], audience: 'all', eastBay: false, nonPriority: true, sort: 'score', dir: 'desc' };
...
export function fromParams(sp: URLSearchParams): Filters {
  return { q: sp.get('q') ?? '', nb: L(sp.get('nb')), type: L(sp.get('type')), tag: L(sp.get('tag')), kitchen: (sp.get('kitchen') as Filters['kitchen']) ?? '', status: L(sp.get('status')), outreach: L(sp.get('outreach')),
    audience: (sp.get('aud') as Filters['audience']) || 'all', eastBay: sp.get('eb') === '1', nonPriority: sp.get('np') !== '0', sort: sp.get('sort') || 'score', dir: (sp.get('dir') as Filters['dir']) || 'desc' };
}
export function toParams(f: Filters): URLSearchParams {
  ...
  if (f.outreach.length) sp.set('outreach', f.outreach.join(',')); if (f.audience !== 'all') sp.set('aud', f.audience); if (f.eastBay) sp.set('eb', '1');
