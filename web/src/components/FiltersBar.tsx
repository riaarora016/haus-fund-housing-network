...
export function FiltersBar({ f, set, rows, showAudience }: { f: Filters; set: (p: Partial<Filters>) => void; rows: Property[]; showAudience: boolean }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 px-2 py-1.5 border-b border-neutral-200 dark:border-neutral-800">
      ...
      <select className="input" value={f.house} onChange={(e) => set({ house: e.target.value as Filters['house'] })} title="which Biopunk house this building could serve"><option value="">house: any</option><option value="punkhaus">punkhaus (40-50)</option><option value="femhaus">femhaus (~12, kitchen)</option><option value="alumhaus">alumhaus / spillover</option><option value="safehaus">safehaus (calm block)</option></select>
      <label className="flex items-center gap-1 text-[12px]" title="no retrofit, no permitting: we can move people in as-is"><input type="checkbox" checked={f.ready} onChange={(e) => set({ ready: e.target.checked })} />Walk-in ready only</label>
