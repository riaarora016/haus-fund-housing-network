...
import { Kitchen, Status, Tag, Source, Sept15, ScoreCell, Fresh, Phone, Email, HouseChip, Channel, Ready } from './Badges';
  ...
  return (
    <aside className="w-[34rem] max-w-full shrink-0 border-l border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 overflow-auto h-full">
      <div className="sticky top-0 bg-white dark:bg-neutral-950 border-b border-neutral-200 dark:border-neutral-800 px-3 py-2 flex items-start gap-2">
        <div className="grow"><div className="font-medium text-[14px]">{p.name}</div>{p.name_detail && <div className="text-[11px] text-neutral-500 italic">{p.name_detail}</div>}<div className="text-neutral-500">{p.address}</div>
          <div className="mt-1 flex flex-wrap gap-1 items-center"><Status s={p.status} /><Source p={p} />{p.timeline_tags.map((x) => <Tag key={x} t={x} />)}{p.aau && <span className="chip border-rose-300 text-rose-700">AAU - pitch as block</span>}{p.houses.map((h) => <HouseChip key={h} h={h} />)}<Channel c={p.deal_channel} /><Ready v={p.walk_in_ready} />{p.baseline && <span className="chip border-sky-300 text-sky-700">BASELINE</span>}<ScoreCell p={p} adj={adjScore(p)} /></div></div>
      ...
      <div className="p-3 space-y-3">
        ...
        {!p.baseline && (() => {
          const base = all.find((x) => x.baseline && x.audience === 'pipeline' && x.type === 'co-living');
          if (!base) return null;
          const w = p.walk_min_from_frontier ?? p.transit_min_to_frontier, bw = base.walk_min_from_frontier ?? base.transit_min_to_frontier;
          const price = p.price_now ?? p.price_per_bed_est, bprice = base.price_per_bed_est;
          const d = (v: number) => (v > 0 ? `+${v}` : String(v));
          const kY = (k: string) => k === 'communal' || k === 'private';
          return (
            <div className="rounded border border-sky-300/60 bg-sky-50 dark:bg-sky-950/30 px-2 py-1">
              <span className="text-[10px] uppercase text-sky-700 dark:text-sky-300">vs the current deal (Fitzgerald co-living, {fmtMoney(bprice)}/bed, ~20 beds, no real kitchen, Sept 27 start)</span>
              <div className="flex flex-wrap gap-x-4">
                <span>price: {price != null && bprice != null ? <b className={price <= bprice ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}>{d(price - bprice)}/bed</b> : 'unknown'}</span>
                <span>kitchen: {kY(p.kitchen) ? <b className="text-emerald-700 dark:text-emerald-400">yes (Fitzgerald: no)</b> : p.kitchen === 'none' ? <b className="text-red-600 dark:text-red-400">also none</b> : 'unknown'}</span>
                <span>beds: {p.beds_est != null ? <b className={p.beds_est >= 40 ? 'text-emerald-700 dark:text-emerald-400' : ''}>{p.beds_est} vs {base.beds_est}</b> : 'unknown'}</span>
                <span>walk: {w != null && bw != null ? <b className={w <= bw ? 'text-emerald-700 dark:text-emerald-400' : ''}>{d(w - bw)} min</b> : 'unknown'}</span>
                <span>Sept 15: {p.sept15_ready === true ? <b className="text-emerald-700 dark:text-emerald-400">yes, beats the Sept 27 start</b> : String(p.sept15_ready)}</span>
              </div>
            </div>
          );
        })()}
        ...
        {p.aau ? (
          <div className="rounded border border-rose-300/60 bg-rose-50 dark:bg-rose-950/30 px-2 py-1 text-[12px]">
            <b>AAU rule:</b> person-to-person only, top-down. Approach as a representative of a large accelerator program. Never mention the residency, never let the automation email them. Elliot is also working the CEO route via his real-estate contact.
          </div>
        ) : (
          <div className="rounded border border-neutral-300/60 dark:border-neutral-700 px-2 py-1 text-[12px] text-neutral-600 dark:text-neutral-400">
            <b>On the call:</b> ask for a block of beds from Sept 15, monthly rate per bed, kitchen access, minimum term.
            If they worry about tenant rights: 3-month program, visa-bound residents, everyone vacates at program end.
          </div>
        )}
