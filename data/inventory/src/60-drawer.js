...
(function () {
  ...
  HFI.drawer = {
    open(id) {
      const r = HFI.record(id); if (!r) return; const f = HFI.filters; const fit = r.fit || HFI.hausFit(r, HFI.ctx()); const cohort = HFI.cohorts.get(f.cohort);
      const d = $('#drawer'); d.classList.add('open'); d.setAttribute('aria-hidden', 'false'); document.body.classList.add('drawer-open');
      const haus = (r.haus_node_ids || []).map((n) => HFI.nodes.get(n)?.name).filter(Boolean);
      const tl = HFI.timeline(r);
      const contactLine = [r.contact_phone ? `<a class="btn sm" href="tel:${r.contact_phone.replace(/[^\d+]/g, '').slice(0, 12)}">Call</a>` : '', r.contact_email ? `<a class="btn sm" href="mailto:${r.contact_email}?subject=${encodeURIComponent(`Housing for ${cohort?.name || 'our cohort'} from ${HFI.fmtDate(f.from)}`)}">Email</a>` : '', r.website_url ? `<a class="btn sm" href="${esc(r.website_url)}" target="_blank" rel="noopener">Website ↗</a>` : ''].join('');
      ...
      <div class="d-head"><button class="x" id="d-close" aria-label="Close">×</button>
        ...
        <div class="d-status"><span class="st ${HFI.statusMeta(r.deal_status).tone}">${HFI.statusMeta(r.deal_status).label}</span><span class="fit big">Fit <b>${fit.score}</b>/100</span>${r.current_deal ? '<span class="tag deal">current deal</span>' : ''}${haus.map((h) => `<span class="tag haus">${esc(h)}</span>`).join('')}</div>
      ...
      <div class="d-body">
        ...
        ${sec('Location', kv([['Walk to Frontier', r.walk != null ? `${r.walk} min${r.walk_is_estimate ? ' <span class="unk">(est. from distance)</span>' : ` <span class="unk">(${esc(r.walk_source || 'tracker')})</span>`}` : val(null)], ['Straight-line', r.distance_miles != null ? `${r.distance_miles} mi` : val(null)], ['Neighborhood', `${val(r.neighborhood)}${r.priority_neighborhood ? ' <span class="tag ok">preferred</span>' : ''}${r.east_bay ? ' <span class="tag warn">East Bay</span>' : ''}`], ['Safety signal', `${({ ok: 'calm block', mixed: 'mixed block', 'rough-block': 'rough block' }[r.safety_flag] || 'no signal')}${r.review_rating ? ` · reviews: ${esc(r.review_rating)}` : ''}${r.safety_note ? `<div class="unk">${esc(r.safety_note)}</div>` : ''}<div class="unk">source: ${esc(r.safety_source)}</div>`]]))}
        ${sec('Operations', kv([['Available from', r.available_from ? HFI.fmtDate(r.available_from) : r.sept15_ready === true ? 'ready for arrival day (tracker)' : r.sept15_ready === false ? 'not ready for arrival day' : val(null)], ['Min / max stay', `${r.minimum_stay_days != null ? r.minimum_stay_days + ' nights' : val(null)} / ${val(r.maximum_stay_days)}`], ['Cohort', cohort ? `${esc(cohort.name)} from ${HFI.fmtDate(cohort.start_date)}: ${fit.parts.timing.note}` : val(null)], ['Bookable online', r.bookable_online ? 'yes' : 'no'], ['Walk-in ready', val(r.walk_in_ready === 'unknown' ? null : r.walk_in_ready)]]))}
    ...
    action(id, act) {
      const r = HFI.record(id); const f = HFI.filters; const cohort = HFI.cohorts.get(f.cohort);
      ...
      if (act === 'copy-outreach') { const txt = HFI.outreachText(r, cohort, f); navigator.clipboard?.writeText(txt); HFI.toast('Outreach text copied'); }
    ...
    edit(id) {
      ...
      d.querySelector('.d-body').innerHTML = `<form id="edit-form" class="edit">
        ...
        ${inp('contact_name')}${inp('contact_phone')}${inp('contact_email', 'email')}${inp('assigned_owner')}
        ...
        <label>assigned cohort<select name="cohort"><option value="">unassigned</option>${opt([...HFI.cohorts.values()].map((c) => ({ id: c.id, label: c.name })), r.cohort_ids?.[0] || '')}</select></label>
      ...
      $('#edit-form').addEventListener('submit', (e) => { e.preventDefault(); const fd = new FormData(e.target); const patch = {};
        for (const [k, v] of fd.entries()) { if (k === 'cohort') { patch.cohort_ids = v ? [v] : []; continue; } const numeric = ['negotiated_per_bed', 'negotiated_monthly', 'asking_per_bed', 'total_beds', 'available_beds', 'private_rooms', 'shared_rooms', 'minimum_stay_days', 'beds_reserved'].includes(k); patch[k] = v === '' ? null : numeric ? +v : v; }
  ...
  // outreach text: template family by operator type, variables from the record and cohort
  HFI.outreachText = (r, cohort, f) => {
    const who = r.contact_name ? r.contact_name.split(/[,(·-]/)[0].trim() : 'there';
    const need = cohort?.target_beds ? `${Math.min(cohort.target_beds, r.beds || cohort.target_beds)}` : '10 to 20';
    ...
    return `Hi ${who},\n\nI represent Haus (haus.fund), a founder residency in San Francisco. We are placing a cohort of founders from ${HFI.fmtDate(f.from || cohort?.start_date)} ${cohort?.start_date ? cohort.start_date.slice(0, 4) : ''} for ${f.nights || 90}+ nights and are looking at ${r.property_name} for a ${kind} of about ${need} beds.\n\nCould you tell me: beds available for those dates, the monthly rate per bed for a block, kitchen access, and minimum term? We decide quickly and pay as a credit tenant.\n\nThanks,\n`;
  };
