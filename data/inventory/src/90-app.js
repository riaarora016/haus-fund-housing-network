...
(function () {
  ...
  HFI.select = (id, opts = {}) => {
    ...
    if (id && !id.startsWith('node:')) { const r = HFI.record(id); if (!r) return; if (opts.fly && r.lat != null && HFI.map) HFI.map.flyTo(r.lat, r.lng, 8); HFI.drawer.open(id); if (changed) HFI.pushUrl(false); }
    else if (id && id.startsWith('node:')) { const n = HFI.nodes.get(id.slice(5)); S.selected = null; HFI.setView('haus'); if (n?.lat != null && HFI.map) HFI.map.flyTo(n.lat, n.lng, 6); }
  ...
  HFI.runPrompt = (text) => { $('#q').value = text; const { filters, read } = HFI.parsePrompt(text, { ...HFI.defaultFilters(), market: HFI.filters.market, sort: HFI.filters.sort, targetPrice: HFI.filters.targetPrice }); const marketChanged = filters.market !== HFI.filters.market; HFI.filters = filters; if (marketChanged) buildMap(); HFI.ui.renderAll(); if (S.results.length && HFI.map) HFI.map.fitPoints(S.results.filter((r) => r.lat != null && !r.east_bay), 0.14); if (!read.length && text.trim()) HFI.toast('Could not read a requirement in that; try "40 beds, kitchen, under $1,000"'); };

  function buildMap() {
    const f = HFI.filters; const market = HFI.markets.get(f.market); const T = HFI.TILES[f.market];
    if (HFI.map) { HFI.map.destroy(); HFI.map = null; }
    $$('[data-market]').forEach((b) => b.setAttribute('aria-pressed', String(b.dataset.market === f.market)));
    if (!T) { $('#mapwrap').innerHTML = `<div class="nomap">No map imagery loaded for ${HFI.esc(market?.name || f.market)} yet.</div>`; return; }
    HFI.map = HFI.createMap($('#mapwrap'), T, { anchor: market.anchor, neighborhoods: HFI.NEIGHBORHOODS[f.market] });
    ...
    const mode = HFI.map.setMode(HFI.prefs.mode || 'sat'); $$('[data-mode]').forEach((x) => { x.setAttribute('aria-pressed', String(x.dataset.mode === mode)); x.disabled = x.dataset.mode === 'map' && !HFI.map.hasStreet(); });
    $$('[data-ovl]').forEach((cb) => { cb.checked = HFI.map.getOverlays()[cb.dataset.ovl]; });
    $('#go-anchor').textContent = market.anchor?.name || 'Center';
    if (market.anchor) HFI.map.flyTo(market.anchor.lat, market.anchor.lng, 4); else HFI.map.home();
  }
  ...
  function init() {
    ...
    $$('[data-market]').forEach((b) => b.addEventListener('click', () => { if (HFI.filters.market === b.dataset.market) return; HFI.filters = { ...HFI.defaultFilters(), market: b.dataset.market, sort: HFI.filters.sort }; $('#q').value = ''; S.selected = null; HFI.drawer.close(); buildMap(); HFI.ui.renderAll(); }));
    buildMap();
    ...
    $('#fit-inv').addEventListener('click', () => { if (!HFI.map) return; const pts = S.results.filter((r) => r.lat != null && !r.east_bay); if (pts.length) HFI.map.fitPoints(pts); else { const a = HFI.markets.get(HFI.filters.market)?.anchor; if (a) HFI.map.flyTo(a.lat, a.lng, 4); } });
    $('#go-anchor').addEventListener('click', () => { const a = HFI.markets.get(HFI.filters.market)?.anchor; if (a && HFI.map) HFI.map.flyTo(a.lat, a.lng, 6); });
    $$('[data-mode]').forEach((b) => b.addEventListener('click', () => { if (!HFI.map) return; const m = HFI.map.setMode(b.dataset.mode); $$('[data-mode]').forEach((x) => x.setAttribute('aria-pressed', String(x.dataset.mode === m))); HFI.prefs.mode = m; HFI.savePrefs(); }));
    ...
    else if (HFI.map && S.results.length) HFI.map.fitPoints(S.results.filter((r) => r.lat != null && !r.east_bay), 0.14);
