...
const STATUS = (v: string): Status => (['active','dark','receivership','on-market','taken','sold','stale-verify','ruled-out'].includes(v) ? (v as Status) : 'active');
...
function blank(): Property {
  return {
    id: '', name: '', address: '', neighborhood: '', neighborhood_raw: '', region: 'SF-other', east_bay: false, priority_neighborhood: false,
    ...
    status: 'active', timeline_tags: [], aau: false, baseline: false, sept15_ready: 'unknown', cluster_id: null, related_id: null, tier: '',
...
function main() {
  ...
  const ordered = props.map((p) => Object.fromEntries(SHEET_COLUMNS.map((c) => [c, (p as any)[c]])));
  fs.writeFileSync(OUT, JSON.stringify(ordered, null, 2) + '\n');
  fs.writeFileSync(path.join(ROOT, 'data', 'join-log.md'), log.join('\n') + '\n');
  fs.writeFileSync(path.join(ROOT, 'data', 'contacts.json'), JSON.stringify(contacts.map(({ key, ...c }) => c), null, 2) + '\n');
